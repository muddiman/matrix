...
export const Matrix = function () {
    this.init           = function () {
                            this.streamListObj = new ListOfStreamersClass();                      
                        };
    this.update         = function () {
                          for (let i=0; i<this.streamListObj.length; i++) {
                           // (this.streamListObj[i]) => {
                            this.streamListObj[i].headPosition = (this.streamListObj[i].headPosition + this.streamListObj[i].speedSetting/100) % (maxROWS + this.streamListObj[i].streamLength);
                              if (this.streamListObj[i].headPosition === 0) {
                                this.streamListObj[i].init();             
                              }    
                           }
                          };   
};
