...
export const Display = function (_color) {
    ...
    this.render     = function (streams) {
                        ...
                        for (let i=0;i<maxSTREAMERS;i++) {                  
                            for (streams[i].row = 0; streams[i].row <= streams[i].headPosition; streams[i].row++) {
                                ...
                                if (streams[i].speedSetting <= 35) {                                      // Slow moving streams get a darker color to give the effect that its further away
                                    this.screen.CONTEXT.fillStyle = `rgba(0, 0, 0,  ${OPAQUE})`;              // BLACK, opaque
                                    this.screen.CONTEXT.font = `0px ${FONT}`;
                        
                                } else {
                                    if (streams[i].speedSetting > 35 && streams[i].speedSetting < 50) { // Slow moving streams get a darker color to give the effect that its further away
                                        this.screen.CONTEXT.fillStyle = `rgba(0, 125, 0,  ${OPAQUE})`;        // DARK green, opaque
                                    }
                                    if (streams[i].headPosition > streams[i].streamLength) {
                                        if (streams[i].row < streams[i].headPosition - streams[i].streamLength) {
                                            this.screen.CONTEXT.fillStyle = `rgba(0, 0, 0,  ${OPAQUE})`;      // BLACK, opaque
                                            this.screen.CONTEXT.font = `0px ${FONT}`;
                                        }
                                    }
                                    if (streams[i].row > streams[i].headPosition-4) {
                                        this.screen.CONTEXT.fillStyle = `rgba(127, 255, 127,  ${OPAQUE})`;    // light green, opaque
                                        this.screen.CONTEXT.font = `${FONT_SIZE13}px ${FONT}`;
                                    }
                                    if  (streams[i].row > streams[i].headPosition-2) {
                                        this.screen.CONTEXT.fillStyle = `rgba(255, 255, 255,  ${OPAQUE})`;    // white, opaque
                                        this.screen.CONTEXT.font = `${FONT_SIZE14}px ${FONT}`;          
                                    }
                                }               
                                this.screen.CONTEXT.fillText(streams[i].stream[streams[i].row], i * 10, (streams[i].row * 15));
                            } 
                        }
                    };
...
/*
function displayStream(streams[i], line) {
    for (streams[i].row = 0; streams[i].row <= streams[i].headPosition; streams[i].row++) {
        this.screen.CONTEXT.font = `${FONT_SIZE}px ${FONT}`;
        ...
        this.screen.CONTEXT.fillText(streams[i].stream[streams[i].row], line * 10, (streams[i].row * 15));
    }
    streams[i].headPosition = (streams[i].headPosition + streams[i].speedSetting/100) % maxROWS;
    if (streams[i].headPosition === 0) {
        streams[i].init();             
    }
}
*/
